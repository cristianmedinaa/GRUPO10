...
public class Biblioteca{
    ...
    public int cantidadSociosPorTipos(String p_objeto){
        ...
        for(Socio s : this.getArraySocios()){
            if(s.soyDeLaClase().equals(p_objeto)){//utliza el metodo soyDeLaClase() para identificar el tipo
                cantSoc++;
            }
        }
        return cantSoc;
    ...
    */
    ...
    public String listaDeSocios(){
        ...
        "\n***************************************************";// ¡¡IMPORTANTE! agreagar el tipo del cual es, no texto 
    ...
    public Socio buscarSocio(int p_dni){
        for(Socio s : this.getArraySocios()){
            if(s.getDniSocio() == p_dni){//si existe devuelve el socio
                return s;
            }else{//si no existe, devuelve una leyenda
    ...
    public String listaDeLibros(){
        for(Libro l : this.getArrayLibros()){
            if(l.prestado()){
                return l.toString() + " || Prestado: (Si)";
            }else{
                return l.toString() + " || Prestado: (No)";
            }
        }
        return "\n";
    }
    ...
    public String listaDeTitulos(){
        ...
        int cantTotal = 0;
        for(Libro l : this.getArrayLibros()){
            l.toString(); //VER SI AGREGAMOS CONDICION PRESTADO() == FALSE
            cantTotal++;
        }
